...
DEFAULT_LOGO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "logo1.jpg")
if os.path.exists(DEFAULT_LOGO_PATH):
    with open(DEFAULT_LOGO_PATH, "rb") as _f:
        logo_url = "data:image/jpeg;base64," + base64.b64encode(_f.read()).decode()
else:
    logo_url = ""
...
_DEFAULT_NOTICE_SCHEDULE = (
    "• 대상자 등록 마감: {enrollment_deadline}\n"
    "• 재심사 결과 식약처 제출 예정일에 맞춰 등록 완료 협조 부탁드립니다."
)
_DEFAULT_NOTICE_SCHEDULE_NOTE = (
    "※ 식약처 정기보고(7년차) 일정\n"
    "2026년 11월 9일, 식약처에 7년차 정기보고 제출 예정입니다. "
    "해당 대상자들의 Data query 해결을 위해 담당 CRA가 기관을 방문 예정이오니 "
    "참고 부탁드리며, query 해결시 많은 협조 부탁드리겠습니다.\n\n"
    "일정 / Timeline\n"
    "• Data Cut-off (정기보고 대상자 확정): ~2026.06.01\n"
    "• Data review & query resolution: ~2026.07.17\n"
    "• Data Management & Statistical Analysis: ~2026.08.27\n"
    "• 7년차 정기보고서 작성 및 식약처 제출: ~2026.11.09"
)
_DEFAULT_NOTICE_ENROLLMENT = (
    "• 조사 동의 이후 등록 가능 / 복용 이력이 있는 환자도 동의 후 등록 가능\n"
    "• 돌연변이 유전자 종류 관계없이 모두 참여 가능 (on set 기간 기준 없음)\n"
    "• 락손필름코팅정 투여 후 1년 시점까지 추적관찰 권장"
)
_DEFAULT_NOTICE_CRF = (
    "• 동의 획득 대상자 정보 E-CRF 입력 요청\n"
    "• 이상사례는 락손 투여시작일 ~ 조사완료 시점까지 지속 수집\n"
    "• 방문 후 정확하고 시기적절하게 작성 요망"
)
...
def apply_theme_palette(html: str, primary: str, secondary: str, accent: str, bg: str, footer: str):
    """미리보기 컬러 팔레트 적용(이메일 HTML 치환)."""
    # 현재 MJML 템플릿의 레드 톤 + 과거 블루 톤 모두 치환
    replacements = {
        "#b71c1c": secondary,
        "#c62828": primary,
        "#ef5350": accent,
...
if menu == "📨 뉴스레터 생성/발송":
    ...
    with col_left:
        ...
        # ── 계절 이미지 ──
        with st.expander("🌿 계절 이미지", expanded=False):
            auto_url = get_season_image_url(greeting_month)
            st.caption(f"현재 {greeting_month}월 자동 이미지 (AI 제공). 직접 업로드하면 자동 이미지를 대체합니다.")
            col_ai, col_up = st.columns([1, 1])
            with col_ai:
                st.image(auto_url, caption=f"{greeting_month}월 자동 이미지", width=160)
            with col_up:
                season_img_file = st.file_uploader(
                    "직접 업로드 (jpg/png/gif)",
                    type=["jpg", "jpeg", "png", "gif"],
                    key="season_img_main",
                )
                if season_img_file:
                    img_bytes = season_img_file.getvalue()
                    mime = season_img_file.type or "image/png"
                    st.session_state["main_season_url"] = (
                        f"data:{mime};base64,{base64.b64encode(img_bytes).decode()}"
                    )
                    st.success(f"✅ {season_img_file.name} 업로드됨")
                    st.image(season_img_file, caption="업로드 이미지", width=160)
            if "main_season_url" in st.session_state:
                if st.button("↩️ 자동 이미지로 복원", key="clear_main_season"):
                    del st.session_state["main_season_url"]
                    st.rerun()
        ...
        # ── 안내사항 ──
        with st.expander("📋 안내사항 내용", expanded=False):
            st.caption("각 항목의 내용을 직접 수정할 수 있습니다.")
            notice_schedule = st.text_area(
                "1. 조사 일정 내용",
                value=_DEFAULT_NOTICE_SCHEDULE.format(enrollment_deadline=enrollment_deadline),
                height=90,
                key="notice_schedule",
            )
            notice_schedule_note = st.text_area(
                "조사 일정 – 추가 안내 (식약처 정기보고 등)",
                value=_DEFAULT_NOTICE_SCHEDULE_NOTE,
                height=180,
                key="notice_schedule_note",
            )
            notice_enrollment = st.text_area(
                "2. 대상자 등록/관찰 내용",
                value=_DEFAULT_NOTICE_ENROLLMENT,
                height=100,
                key="notice_enrollment",
            )
            notice_crf = st.text_area(
                "3. CRF 수집 내용",
                value=_DEFAULT_NOTICE_CRF,
                height=100,
                key="notice_crf",
            )
            st.markdown("**📎 CRF Completion Guideline 첨부파일**")
            st.caption("업로드 시 'CRF Completion Guideline 첨부 참고' 문구에 자동으로 링크가 연결됩니다.")
            crf_file = st.file_uploader(
                "CRF Guideline 파일 업로드 (pdf/docx/xlsx 등)",
                type=["pdf", "docx", "doc", "xlsx", "xls", "pptx", "ppt"],
                key="crf_guideline_file",
            )
            if crf_file:
                att_bytes = crf_file.getvalue()
                att_ext = os.path.splitext(crf_file.name)[-1]
                att_tmp = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"_tmp_crf{att_ext}")
                with open(att_tmp, "wb") as _f:
                    _f.write(att_bytes)
                st.session_state["crf_attachment_path"] = att_tmp
                st.session_state["crf_attachment_name"] = crf_file.name
                mime = crf_file.type or "application/octet-stream"
                st.session_state["crf_guideline_preview_url"] = (
                    f"data:{mime};base64,{base64.b64encode(att_bytes).decode()}"
                )
                st.success(f"✅ {crf_file.name} 업로드됨")
            if "crf_attachment_name" in st.session_state:
                st.info(f"현재 첨부: {st.session_state['crf_attachment_name']}")
                if st.button("🗑️ 첨부파일 제거", key="clear_crf"):
                    for k in ["crf_attachment_path", "crf_attachment_name", "crf_guideline_preview_url"]:
    ...
    with col_right:
        ...
        has_prev = "last_data" in st.session_state
        ...
        if generate_btn or send_btn:
            ...
            data = {
                ...
                "greeting_image": st.session_state.get("main_season_url", get_season_image_url(month)),
                "schedule_image": schedule_image,
                "has_enrollment": site_enrolled > 0,
                "logo_url": logo_url,
                "notice_schedule": st.session_state.get("notice_schedule", _DEFAULT_NOTICE_SCHEDULE.format(enrollment_deadline=enrollment_deadline)),
                "notice_schedule_note": st.session_state.get("notice_schedule_note", _DEFAULT_NOTICE_SCHEDULE_NOTE),
                "notice_enrollment": st.session_state.get("notice_enrollment", _DEFAULT_NOTICE_ENROLLMENT),
                "notice_crf": st.session_state.get("notice_crf", _DEFAULT_NOTICE_CRF),
                "crf_guideline_link": st.session_state.get("crf_guideline_preview_url", ""),
            }
            st.session_state["last_data"] = data
            has_prev = True

        if has_prev:
            base_data = st.session_state["last_data"]

            # 새 생성 시(생성 버튼) 편집 적용값 초기화
            if generate_btn or "applied_greeting_body" not in st.session_state:
                st.session_state["applied_greeting_body"] = base_data.get("greeting_body", "")
                st.session_state["applied_sender_name"] = base_data.get("sender_name", "")
                st.session_state["applied_greeting_image"] = base_data.get("greeting_image", "")
                st.session_state["applied_theme_primary"] = "#c62828"
                st.session_state["applied_theme_secondary"] = "#b71c1c"
                st.session_state["applied_theme_accent"] = "#ef5350"
                st.session_state["applied_theme_bg"] = "#fff5f5"
                st.session_state["applied_theme_footer"] = "#8e0000"

            st.session_state.setdefault("applied_theme_footer", "#8e0000")

            # draft 값 초기화
            if generate_btn:
                st.session_state["draft_greeting_body"] = st.session_state["applied_greeting_body"]
                st.session_state["draft_sender_name"] = st.session_state["applied_sender_name"]
                st.session_state["draft_greeting_image"] = st.session_state["applied_greeting_image"]
                st.session_state["draft_theme_primary"] = st.session_state["applied_theme_primary"]
                st.session_state["draft_theme_secondary"] = st.session_state["applied_theme_secondary"]
                st.session_state["draft_theme_accent"] = st.session_state["applied_theme_accent"]
                st.session_state["draft_theme_bg"] = st.session_state["applied_theme_bg"]
                st.session_state["draft_theme_footer"] = st.session_state["applied_theme_footer"]
            else:
                st.session_state.setdefault("draft_greeting_body", st.session_state["applied_greeting_body"])
                st.session_state.setdefault("draft_sender_name", st.session_state["applied_sender_name"])
                st.session_state.setdefault("draft_greeting_image", st.session_state.get("applied_greeting_image", base_data.get("greeting_image", "")))
                st.session_state.setdefault("draft_theme_primary", st.session_state["applied_theme_primary"])
                st.session_state.setdefault("draft_theme_secondary", st.session_state["applied_theme_secondary"])
                st.session_state.setdefault("draft_theme_accent", st.session_state["applied_theme_accent"])
                st.session_state.setdefault("draft_theme_bg", st.session_state["applied_theme_bg"])
                st.session_state.setdefault("draft_theme_footer", st.session_state["applied_theme_footer"])

            with st.expander("🎨 미리보기 편집 (문구/색상)", expanded=True):
                st.caption("값을 바꾼 뒤 아래 [반영] 버튼을 눌러야 미리보기에 적용됩니다.")

                with st.form("preview_edit_form"):
                    st.text_area("인사말 본문", key="draft_greeting_body", height=180)
                    st.text_input("발신자명", key="draft_sender_name")

                    c1, c2 = st.columns(2)
                    with c1:
                        st.color_picker("메인 컬러", key="draft_theme_primary")
                        st.color_picker("보조 컬러", key="draft_theme_secondary")
                    with c2:
                        st.color_picker("강조 컬러", key="draft_theme_accent")
                        st.color_picker("배경 컬러", key="draft_theme_bg")
                        st.color_picker("푸터 컬러", key="draft_theme_footer")

                    apply_btn = st.form_submit_button("🎯 반영", use_container_width=True)

                if apply_btn:
                    st.session_state["applied_greeting_body"] = st.session_state["draft_greeting_body"]
                    st.session_state["applied_sender_name"] = st.session_state["draft_sender_name"]
                    st.session_state["applied_greeting_image"] = st.session_state["draft_greeting_image"]
                    st.session_state["applied_theme_primary"] = st.session_state["draft_theme_primary"]
                    st.session_state["applied_theme_secondary"] = st.session_state["draft_theme_secondary"]
                    st.session_state["applied_theme_accent"] = st.session_state["draft_theme_accent"]
                    st.session_state["applied_theme_bg"] = st.session_state["draft_theme_bg"]
                    st.session_state["applied_theme_footer"] = st.session_state["draft_theme_footer"]
                    st.success("✅ 미리보기에 반영되었습니다.")

            preview_data = dict(base_data)
            preview_data["greeting_body"] = st.session_state["applied_greeting_body"]
            preview_data["sender_name"] = st.session_state["applied_sender_name"]
            preview_data["sender_pm"] = st.session_state["applied_sender_name"]
            preview_data["greeting_image"] = st.session_state.get("applied_greeting_image", base_data.get("greeting_image", ""))
            preview_data["theme"] = {
                "primary": st.session_state["applied_theme_primary"],
                "secondary": st.session_state["applied_theme_secondary"],
                "accent": st.session_state["applied_theme_accent"],
                "bg": st.session_state["applied_theme_bg"],
                "footer": st.session_state["applied_theme_footer"],
            }

            html_output = render_html(preview_data)
            html_output = apply_theme_palette(
                html_output,
                st.session_state["applied_theme_primary"],
                st.session_state["applied_theme_secondary"],
                st.session_state["applied_theme_accent"],
                st.session_state["applied_theme_bg"],
                st.session_state["applied_theme_footer"],
            )

            st.session_state["last_html"] = html_output
            st.success("✅ 생성 완료! (편집 반영)")
            st.markdown(download_link(html_output, f"newsletter_{preview_data['hospital']}.html"), unsafe_allow_html=True)
            st.divider()
            ...
            if send_btn:
                if not to_email:
                    ...
                    subject = f"[{preview_data['study_id']}] 락손필름코팅정 사용성적조사 News Letter"
                    img_path = st.session_state.get("schedule_image_path", "")
                    result = send_newsletter(
                        to_email, preview_data["professor"], subject, html_output,
                        smtp_host=smtp_host, smtp_port=int(smtp_port),
                        smtp_user=smtp_user, smtp_pass=smtp_pass,
                        template_data=preview_data,
                        image_path=img_path,
                        logo_path=DEFAULT_LOGO_PATH,
                        attachment_path=st.session_state.get("crf_attachment_path", ""),
                    )
                    if result["ok"]:
                        log_send(preview_data["hospital"], preview_data["professor"], to_email, "sent")
                        st.success(f"✅ 발송 완료! → {to_email}")
                    else:
                        log_send(preview_data["hospital"], preview_data["professor"], to_email, f"error: {result['error']}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    tab1, tab2 = st.tabs(["📋 DB 목록", "📤 엑셀 업로드"])
    ...
    with tab1:
        ...
        with st.expander("➕ 기관 직접 추가", expanded=False):
            with st.form("add_hospital_form"):
                a1, a2 = st.columns(2)
                add_hospital = a1.text_input("병원명", key="add_hospital_name")
                add_professor = a2.text_input("교수명", key="add_professor")
                add_email = st.text_input("이메일", key="add_email")
                ac1, ac2, ac3, ac4 = st.columns(4)
                add_contracted = ac1.number_input("계약례", min_value=0, value=0, key="add_contracted")
                add_enrolled = ac2.number_input("등록례", min_value=0, value=0, key="add_enrolled")
                add_ecrf = ac3.number_input("ECRF작성례", min_value=0, value=0, key="add_ecrf")
                add_delta = ac4.number_input("전월대비변동", value=0, key="add_delta")
                add_submit = st.form_submit_button("💾 기관 추가", type="primary", use_container_width=True)

            if add_submit:
                if not add_hospital.strip() or not add_professor.strip() or not add_email.strip():
                    st.error("병원명, 교수명, 이메일은 필수 입력입니다.")
                else:
                    upsert_hospital(
                        hospital_name=add_hospital.strip(),
                        professor=add_professor.strip(),
                        email=add_email.strip(),
                        site_contracted=int(add_contracted),
                        site_enrolled=int(add_enrolled),
                        site_ecrf=int(add_ecrf),
                        site_delta=int(add_delta),
                    )
                    st.success(f"✅ '{add_hospital.strip()}' 기관이 추가되었습니다.")
                    st.rerun()
